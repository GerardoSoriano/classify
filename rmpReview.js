...
async function getReviewForProfessor(professorName) {
  ...
  const listItems = await ratingList.$$eval('li', lis => lis.map(li => {
        ...
        const numQuality = li.querySelector('.CardNumRating__CardNumRatingNumber-sc-17t4b9u-2');
        const numDifficulty = li.querySelector('.CardNumRating__CardNumRatingNumber-sc-17t4b9u-2.cDKJcc');
        const classLabel = li.querySelector('.RatingHeader__StyledClass-sc-1dlkqw1-3.eXfReS');
        const labels = Array.from(li.querySelectorAll('.RatingTags__StyledTags-sc-1boeqx2-0.eLpnFv > .Tag-bs9vf4-0.hHOVKF')).map(span => span.textContent);
        ...
        return {
            ...
            numDifficulty: numDifficulty ? parseFloat(numDifficulty.textContent) : null,
            classLabel: classLabel ? classLabel.textContent : null,
            label1: label1,
            label2: label2,
            label3: label3
        }
      }).filter(item => item.reviewText && item.numQuality && item.numDifficulty && item.classLabel && item.label1 && item.label2 && item.label3));

    const reviews = listItems.map(item => new Review(item.reviewText, item.numQuality, item.numDifficulty, item.classLabel, item.label1, item.label2, item.label3));
